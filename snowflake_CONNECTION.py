...
# Constants from Airflow Variables (better practice than hardcoding)
SNOWFLAKE_CONN = "snowflake_conn"
SYMBOLS = ["NVDA"]  # Added Apple as second stock
FORECASTING_PERIODS = 7  # Number of days to forecast
...
SCHEMA_ML = Variable.get("snowflake_schema_ml", default_var="RAW")  # Using RAW since we have access to it
...
# Task to create Snowflake table if not exists

@task
def create_tables():
    cursor = get_snowflake_cursor()
    try:
        # Create stock price table if not exists
        cursor.execute(f"""
            CREATE TABLE IF NOT EXISTS {DATABASE}.{SCHEMA_RAW}.STOCK_PRICE (
                SYMBOL VARCHAR(10),
                OPEN FLOAT,
                HIGH FLOAT,
                LOW FLOAT,
        ...
        # Create forecast results table if not exists
        cursor.execute(f"""
            CREATE TABLE IF NOT EXISTS {DATABASE}.{SCHEMA_ML}.FORECAST_RESULTS (
                SYMBOL VARCHAR(10),
                DATE DATE,
                FORECAST FLOAT,
                LOWER_BOUND FLOAT,
...
# Task to create the final market data table (union of actual and forecast)
...
def create_final_table():
    ...
    try:
        cursor.execute(f"""
            CREATE OR REPLACE TABLE {DATABASE}.{SCHEMA_ML}.MARKET_DATA_FINAL AS
            ...
            SELECT 
                ...
                LOWER_BOUND,
                UPPER_BOUND
            FROM {DATABASE}.{SCHEMA_ML}.FORECAST_RESULTS
        ...
        print(f"Final table creation failed: {str(e)}")
        raise
    finally:
        cursor.close()
...
# Define the DAGS
with DAG(
    dag_id="stock_price_prediction_pipeline_TEST1",
    ...
    for symbol in SYMBOLS:
        ...
        setup_tables >> load_task >> train_task >> forecast_task
    
    # Final step - create the combined table with both historical and forecast data
    final_table_task = create_final_table()
    
    # Make sure final table is created after all forecasts are done
    for symbol in SYMBOLS:
        # This ensures final_table_task runs after all symbol forecasts are complete
        # We need to get the task instance for the forecast task for each symbol
        forecast_task = dag.get_task("generate_forecast")
        forecast_task.set_downstream(final_table_task)